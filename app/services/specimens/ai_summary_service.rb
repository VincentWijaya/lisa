module Specimens
  class AiSummaryService
    ...
    def call
      ...
      @specimen.update!(ai_summary: content)
      ServiceResult.success(specimen: @specimen, summary: content)
    ...
    def examination_payload
      works = specimen.works.includes(:examination, examination_results: :reference_rule).order(:label_sequence)
      lines = []
      works.each do |work|
        header = "- #{work.examination.name} (Kategori: #{work.examination.category}, Kode: #{work.examination.code})"
        header += " [Label Group: #{work.examination.label_group}]" if work.examination.label_group.present?
        lines << header

        if work.examination_results.empty?
          lines << "  (Belum ada hasil)"
          next
        end

        work.examination_results.order(:id).each do |result|
